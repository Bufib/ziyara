...
| `/admin` | Admin-Session | getrennte Punkte für Busmanagement, Generalalarm, Reiseführung und Mehrziel-Navigation sowie Gruppenabfrage, Fragen und Benutzerübersicht |
...
Mit der lokal vorbereiteten Mehrzielmigration existieren achtzehn Anwendungstabellen im `public`-Schema. Die siebzehn zuvor bestehenden Tabellen bleiben vollständig erhalten; ebenso `profiles.id`, `profiles.user_id`, `group_check_responses.id` und `member_type`.
